 ...
 */
export async function generateVscodeDevAuthState(
	token: string,
): Promise<IPlaywrightStorageState> {
	const authResponse = await fetch("https://auth.insiders.vscode.dev/", {
		method: "POST",
	});

	if (!authResponse.ok) {
		throw new Error(
			`Failed to reach auth endpoint: ${authResponse.status} ${authResponse.statusText}`,
		);
	}
		...
		cookies: [
			{
				...
				path: "/",
				expires:
					new Date(vscodeSessionCookie["expires"]).getTime() / 1000,
				httpOnly: true,
				secure: false,
				sameSite: "Strict",
			},
