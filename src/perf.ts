...
import {
	DATA_FOLDER,
	EXTENSIONS_FOLDER,
	INSIDERS_VSCODE_DEV_HOST_NAME,
	PERFORMANCE_FILE,
	PERFORMANCE_RUNS,
	Runtime,
	USER_DATA_FOLDER,
	VSCODE_DEV_HOST_NAME,
...
export async function launch(options: Options) {
	...
	for (let i = 0; i < runs; i++) {
		console.log(
			`${chalk.gray("[perf]")} running session ${chalk.green(`${i + 1}`)} of ${chalk.green(`${runs}`)}...`,
		);
...
async function preparePlaywright(
	...
	if (
		...
		(url.hostname === VSCODE_DEV_HOST_NAME ||
			url.hostname === INSIDERS_VSCODE_DEV_HOST_NAME)
	) {
...
async function launchWeb(
	...
	playwrightStorageState: IPlaywrightStorageState | undefined,
	signal: AbortSignal,
): Promise<string> {
	const url = new URL(options.build);
	...
	payload.push([
		"profDurationMarkers",
		durationMarker === "ellapsed"
			? "code/timeOrigin,code/didStartWorkbench"
			: durationMarker.split("-").join(","),
	]);
...
async function startHeapTracing(
	cdp: playwright.CDPSession,
): Promise<{ stop: () => Promise<IGCStatistics> }> {
...
async function resolveStartupHeapStatistics(
	cdp: playwright.CDPSession,
	data: ITracingData[],
): Promise<IGCStatistics> {
	...
	return {
		minorGCs,
		majorGCs,
		used: heapSnapshot.used,
		garbage,
		duration: Math.round(duration / 1000),
	};
}

function gcStatisticsToString({
	used,
	garbage,
	majorGCs,
	minorGCs,
...
async function collectHeapSnaptshot(cdp: playwright.CDPSession) {
	...
	return {
		used: usedSizeAfterGC,
		garbage: usedSizeBeforeGC - usedSizeAfterGC,
	};
}
	...
	const median =
		markerDurations.length % 2 === 0
			? (markerDurations[middleIndex - 1] +
					markerDurations[middleIndex]) /
				2
			: markerDurations[middleIndex];

	console.log(
		`${chalk.gray("[perf]")} ${marker}: ${chalk.green(`${duration}ms`)} (current), ${chalk.green(`${markerDurations[0]}ms`)} (fastest), ${chalk.green(`${markerDurations[markerDurations.length - 1]}ms`)} (slowest), ${chalk.green(`${median}ms`)} (median)`,
	);
}
